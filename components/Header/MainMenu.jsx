    ...
    <ul className={`menu__list menu__list--submenu ${isSubMenuOpen && 'menu__list--submenu-open'}`}>
      {menuList.map((item) => (
        <li key={item.id} className="menu__list-item">
          <Link href={`${item.route ?? '#'}`} className="menu__list-link" onClick={() => handleSubmenuBtnClick(item)}>
            {item.title}
      ...
      <ul className={`menu__list ${isMainMenuVisible && 'menu__list--visible'}`}>
        {menu.map((item) => {
          const itemLink = (<li key={item.id} className="menu__list-item">
            {item.route && (<Link href={item.route} className="menu__list-link" onClick={() => handleSubmenuBtnClick(item)}>
              {item.title}
